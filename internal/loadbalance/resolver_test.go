...
func TestResolver(t *testing.T) {
	...
	srv, err := server.NewGRPCServer(
		&server.Config{
			GetServerer: &getServers{}, // pass a mock GetServerers object
		},
	...
	_, err = r.Build(
		// point to the server that we set up above (on which it'll call
		// GetServers() to resolve the servers and update the client)
		resolver.Target{
...
// getServers implements GetServerers interface
// it's job is to return known server set for resolver to find
...
// clientConn implements resolver.ClientConn
// it's job is to keep reference to the state that resolver updated it with,
// so we can verify that resolver updates client connection with correct data
